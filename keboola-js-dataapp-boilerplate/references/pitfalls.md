...
## 30. Prebuild the app — commit `dist/`, don't `npm install`/`build` on every container start

**Symptom / motivation:** Keboola's own support flagged this proactively on a slow-starting app: PROD's `setup.sh` ran `npm install && npm run build` on every container (re)start, reinstalling ~350 packages and rebuilding client+server from scratch each time — a normal cold start (after auto-suspend, or right after a redeploy) took ~20-40s just for `setup.sh`, before the server was even listening. This is "the approach we generally recommend for JavaScript apps," per Keboola support.

**Fix:** commit the built artifact instead of building it at container-start time. `keboola-config/setup.sh` becomes a no-op (see `scaffold.md`), and `dist/` is no longer gitignored. Confirmed result on a real app: `setup.sh` dropped from ~20-30s to **7ms**, and the whole container reached "server listening" in under 2 seconds.

**The trap — do NOT also commit `node_modules/`, even pruned to production-only deps.** The natural first instinct is: commit `dist/` (tsc/vite output) AND a `node_modules` pruned with `npm prune --omit=dev` (production deps only), since the compiled server still does plain `require()` calls into it at runtime. This works locally and passes every smoke test — right up until `git push`, which this git host (`git.<region>.keboola.com`) rejects with **`HTTP 413` ("payload too large")** once the pushed objects get into the several-MB range. A pruned `node_modules` for even a small server (Express + one more real dependency, e.g. `google-auth-library`) was already ~59MB / 130+ packages and got rejected — and so did a single small commit whose *compressed* pack was only ~7.5MB. There is no client-side buffer-size setting (`http.postBuffer`) that fixes this; it's a real server-side request-size limit, and pack compression alone doesn't get you under it once `node_modules` is involved.

**The actual fix: bundle the server with esbuild instead of just transpiling it.** Add `esbuild` as a devDependency and an `esbuild.server.js` script (see `scaffold.md`) that bundles `server/index.ts` — with `express`, `google-auth-library`, and every other server-side dependency inlined — into ONE minified `dist/server/index.js`. The container then needs **no `node_modules` at all** at runtime. Verified on a real app: `express` + `google-auth-library` (a nontrivial dependency tree — `gaxios`, `gtoken`, `gcp-metadata`, `jws`, `jwa`, etc.) bundled to a 1.5MB file with zero runtime issues; smoke-tested by literally renaming `node_modules` out of the way and confirming the bundled server still boots and serves every route (including a Cloud Identity API group-auth gate) correctly. Total `dist/` (client + bundled server) ended up ~2.2MB — comfortably under whatever this host's push-size limit actually is, vs. the ~59MB the node_modules approach would have needed.

**Before deciding to bundle a given dependency, check for native/compiled bindings** (`find node_modules -maxdepth 2 -name binding.gyp`) — a pure-JS dependency tree bundles cleanly and is also portable across your dev machine's OS and the Linux container without a rebuild step; a dependency with a compiled native addon (rare for typical Express/API-client packages, but check) would need `--external:<pkg>` and a different strategy.

**Type-checking still needs `tsc`** — esbuild transpiles but does not type-check (same root cause as §26, just for the *server* half this time instead of the client). Keep `tsc -p tsconfig.server.json --noEmit` in the `build` script purely for type errors; esbuild only does the actual bundling into `dist/server/index.js`.

**Workflow implication — this removes your safety net.** Before this change, even if you forgot to run `npm run build` locally, a `git push` + container restart would still rebuild from source (slowly, but correctly). After this change, PROD's `setup.sh` does nothing — if you push a source change without also running `npm install && npm run build` and committing the resulting `dist/`, the container will happily keep serving the OLD bundled `dist/` forever, with no error anywhere to catch it. Make "rebuild + commit `dist/`" a non-negotiable step of shipping, not an optional one (see Step 7 and the pre-flight checklist in `SKILL.md`).

**`setup-dev.sh` (drafts) is untouched by this** — the dev/draft flow still wants a live `npm install` (no build) for Vite HMR + `tsx watch` iteration; this optimization is PROD-only.
...
**(a) Prod has no hot-reload path.** Without the prebuild optimization (§30), every `deploy_data_app` call on a prod (non-draft) config re-clones, re-installs (no cache between deploys), and rebuilds from scratch — genuinely ~20-40s — then you poll `starting` a few times before it's `running`. With §30 applied, that rebuild cost moves to your machine (once, before each push) and the container itself starts in ~1-2s — but you still pay the round-trip of build → commit → push → redeploy → verify per tweak, which is most of what makes iteration feel slow in a long session (see (b)). A draft deployed once in dev mode (`mode='dev'`) sidesteps both: push a tweak to its branch and the container's `git-watcher` + Vite HMR + `tsx watch` hot-reload in place (dev mode runs `setup-dev.sh`, not the prebuilt PROD path), no `deploy_data_app` call needed, ~1-3s per tweak. **This is a real option, but only take it if the user wants it** — some explicitly prefer every tweak live on the one real prod URL and find a second draft URL/branch more confusing than the rebuild cost is worth. Ask once, then respect the answer; don't re-relitigate it every time a rebuild feels slow.
